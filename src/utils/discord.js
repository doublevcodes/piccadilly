...
module.exports.getCommands = async (applicationId, token, tokenType, guildId = undefined) =>
    api(guildId ? Routes.applicationCommands(applicationId) : Routes.applicationCommands(applicationId), 'GET', token, tokenType)
        .then(res => res.json());

module.exports.registerCommand = async (applicationId, token, tokenType, data, guildId = undefined) =>
    api(guildId ? Routes.applicationCommands(applicationId) : Routes.applicationCommands(applicationId), 'POST', token, tokenType, data)
        .then(res => res.json());

module.exports.updateCommand = async (applicationId, token, tokenType, commandId, data, guildId = undefined) =>
    api(guildId ? Routes.applicationCommands(applicationId, commandId) : Routes.applicationCommand(applicationId, commandId), 'PATCH', token, tokenType, data)
        .then(res => res.json());
